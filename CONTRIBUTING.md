...
Local dev uses **SQLite** by default (`./data/secrets.db`). Firebase env vars are only needed when testing the **GCP / Firestore** path — see [DEPLOYMENT.md](./DEPLOYMENT.md).

## Architecture & Tech Stack

*   **Frontend**: React 19, Tailwind CSS 4, Motion.
*   **Backend**: Node.js (Express) with `helmet` and `express-rate-limit`.
*   **Database**: **SQLite** (default, self-hosted) or **Cloud Firestore** (GCP Cloud Run).
